...
class ImageProcessorThread(QThread):
    """Thread สำหรับทำงานรวมภาพและ Upscaling อยู่เบื้องหลังเพื่อไม่ให้ UI ค้าง"""
    process_finished = pyqtSignal(str)
    process_failed = pyqtSignal(str)

    def __init__(self, template_image_path: str, photo_paths: list[str], layout_config: dict, output_dir: str):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            self.process_finished.emit(output_path)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        # เตรียมตัวแปรสำหรับ Caching QR Code
        self.cached_qr_pixmap = None
        gdrive_id = self.config_manager.get("gdrive_folder_id", "")
        if gdrive_id:
            self._update_qr_cache(gdrive_id)
    ...
    def _update_qr_cache(self, folder_id: str) -> None:
        """สร้างและแคช QR Code สำหรับ Google Drive Folder"""
        if not folder_id:
            self.cached_qr_pixmap = None
            return
        ...
        try:
            ...
            cache_path = os.path.join(save_dir, "qr_cache.png")
            
            # เซฟไฟล์รูปลงดิสก์
            pil_image.save(cache_path)
            
            # โหลดเป็น QPixmap จากไฟล์แทนการแปลงตรงๆ เพื่อป้องกันปัญหา Memory (C++ Assertion/Crash)
            self.cached_qr_pixmap = QPixmap(cache_path)
            
            logger.info("สร้าง Cache QR Code สำหรับ GDrive ID: %s เรียบร้อย", folder_id)
        except Exception as e:
            logger.error("เกิดข้อผิดพลาดในการสร้าง Cache QR Code: %s", e)
            self.cached_qr_pixmap = None
    ...
    def _on_home_logo_clicked(self) -> None:
        ...
        self.finish_widget.setup_finish(self.cached_qr_pixmap, gdrive_folder_id)
    ...
    def _on_capture_finished(self, taken_photos: list[str]) -> None:
        ...
        try:
            # 5. เริ่มรันงานประมวลผลรูป (Upscaling + Merge) ใน QThread
            ...
            self.processor_thread.process_finished.connect(self._on_processing_finished)
            self.processor_thread.process_failed.connect(self._on_processing_failed)
            self.processor_thread.start()
            
            logger.info("กำลังรวมภาพและ Upscaling อยู่ในเบื้องหลัง...")
            # ยังไม่เปลี่ยนหน้า ปล่อยให้หน้า Capture โชว์ "กำลังรวมภาพ..." ต่อไป
            
        except Exception as e:
            logger.error("เกิดข้อผิดพลาดในการเริ่มประมวลผล: %s", e)
            QMessageBox.critical(self, "ข้อผิดพลาด", str(e))
            self.navigate_to(PAGE_HOME)

    def _on_processing_finished(self, output_path: str) -> None:
        """ทำงานเมื่อรวมรูปและ Upscaling ใน ImageProcessorThread เสร็จสมบูรณ์"""
    ...
    def _on_processing_failed(self, err_msg: str) -> None:
        """ทำงานเมื่อเกิดข้อผิดพลาดในการรวมรูปใน Thread"""
        logger.error("เกิดข้อผิดพลาดในการรวมภาพ: %s", err_msg)
        QMessageBox.critical(self, "ข้อผิดพลาดในการรวมภาพ", str(err_msg))
        self.navigate_to(PAGE_HOME)
    ...
    def _on_settings_saved(self, settings: dict) -> None:
        ...
        # อัปเดต QR Code Cache เมื่อมีการบันทึก Settings
        new_gdrive_id = settings.get("gdrive_folder_id", "")
        old_gdrive_id = self.config_manager.get("gdrive_folder_id", "")
        if new_gdrive_id != old_gdrive_id or not self.cached_qr_pixmap:
            self._update_qr_cache(new_gdrive_id)
